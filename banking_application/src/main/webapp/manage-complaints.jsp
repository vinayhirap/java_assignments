<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="com.aurionpro.model.Complaint"%>
...
<title>Manage Complaints - Aurionpro Bank Admin</title>
<link
			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td><span
							class="badge bg-<%=complaint.getStatus().name().toLowerCase()%>"><%=complaint.getStatus()%></span></td>
						<td><%=complaint.getCreatedAt()%></td>
						<td>
							<form action="update-complaint-status" method="post"
								class="d-inline">
								<input type="hidden" name="complaintId"
									value="<%=complaint.getComplaintId()%>" /> <select
									name="status"
									class="form-select form-select-sm d-inline w-auto" required>
									<option value="OPEN"
										<%=complaint.getStatus() == Complaint.ComplaintStatus.OPEN ? "selected" : ""%>>Open</option>
									<option value="IN_PROGRESS"
										<%=complaint.getStatus() == Complaint.ComplaintStatus.IN_PROGRESS ? "selected" : ""%>>In
										Progress</option>
									<option value="RESOLVED"
										<%=complaint.getStatus() == Complaint.ComplaintStatus.RESOLVED ? "selected" : ""%>>Resolved</option>
									<option value="CLOSED"
										<%=complaint.getStatus() == Complaint.ComplaintStatus.CLOSED ? "selected" : ""%>>Closed</option>
								</select>
